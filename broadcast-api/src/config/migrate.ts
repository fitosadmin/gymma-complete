...
//
// Advisory lock (pg_try_advisory_lock) prevents the race condition where
// server.ts and worker.ts restart simultaneously and both try to apply the
// same migration at the same time. Only one process holds the lock at a time;
// the other skips the run entirely (tables are already there).
import { readFileSync, readdirSync } from 'node:fs';
import { join } from 'node:path';
import { pool } from './database';
import { logger } from './logger';

// Arbitrary stable integer used as the advisory lock key.
// Must be the same across server.ts and worker.ts. 1234567890 = "gymma-broadcast"
const MIGRATION_LOCK_KEY = 1_234_567_890;
...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    // Acquire a session-level advisory lock so only one process (server OR worker)
    // runs migrations at a time. If both restart simultaneously, the second one
    // simply returns immediately — the first one's commit will have already
    // applied the SQL and written the tracking row by the time it releases.
    const { rows: lockRows } = await client.query<{ acquired: boolean }>(
      `SELECT pg_try_advisory_lock($1) AS acquired`,
      [MIGRATION_LOCK_KEY],
    );
    if (!lockRows[0]?.acquired) {
      logger.info('migration lock held by another process — skipping (tables will be ready shortly)');
      return;
    }
